...
class Table
  ...
  attr_accessor :ws, :selected_col, :offset_i, :offset_j, :null_rows
  ...
  def []=(column_header, index, val)
    ...
    @ws[offset_i + index+1, offset_j + header_index] = val # dodajemo 1 na indexe zato sto u ws indexiranje krece od 1
    save()
  end
  ...
  def ignore_empty_rows(index)
    index += 1 # dodajemo 1 na indexe zbog reda hedera
    if !@null_rows.empty?
